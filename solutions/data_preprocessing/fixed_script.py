...
def normalize_batch(images):
    ...
    """
    # Fix 1: reshape to (1, C, 1, 1) so it broadcasts correctly over (B, C, H, W)
...
def convert_labels(labels, num_classes=10):
    ...
    return labels.long()  # Fix 2: CrossEntropyLoss wants integer class indices, not one-hot


def augment(images):
    """Randomly flip images horizontally."""
    if torch.rand(1).item() > 0.5:
        images = torch.flip(images, dims=[3])  # Fix 3: dim 3 = width (horizontal flip)
    return images
