...
def load_training_data():
    with open('training_data.json', 'r') as f:  # Fix 1: correct filename
        return json.load(f)
...
def extract_features_and_labels(data):
    ...
    for sample in data:
        ...
        # Fix 2: handle samples that use 'status' key instead of 'approved'
        approved = sample.get('approved', sample.get('status') == 'approved')
        labels.append(1 if approved else 0)
...
def main():
    ...
    features = np.array(all_features, dtype=float)  # Fix 3: dtype=float not str
