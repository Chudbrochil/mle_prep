...
class CNN(nn.Module):
    ...
    def __init__(self, num_classes=10):
        super().__init__()
        self.conv1 = nn.Conv2d(3, 32, kernel_size=5, padding=2)  # Fix 1: padding=2 for kernel=5
    ...
    def forward(self, x):
        ...
        return self.fc3(x)  # Fix 2: return raw logits, no softmax
...
class SmallCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.pool(torch.relu(self.conv1(x)))
        x = self.pool(torch.relu(self.conv2(x)))
        x = x.view(x.size(0), -1)  # Fix 3: use x.size(0), not hardcoded 32
